    ...
    if (Math.abs(xDiff) > 3 || Math.abs(yDiff) > 3) {
      ...
      if (this.offsprings && !this.madeChilds) {
        ...
        for (let i = 0; i < start; i++) {
          ...
          newyear.fireworks.push(new Firework(this.x, this.y, targetX, targetY, this.shade, 0))
...
let canvas = document.getElementById('newyear')
let ctx = canvas.getContext('2d')

let newyear = new FireworksShow
window.onresize = () => newyear.resize()
document.onclick = (evt) => newyear.onClick(evt)
document.ontouchstart = (evt) => newyear.onClick(evt)
